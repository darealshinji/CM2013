...
// FAKEFACTORY SHADER EDITOR
static ConVar ff_ppe_vignette( "ff_ppe_vignette", "1",0,"Enable or disable Vignette postprocessing" );
static ConVar ff_ppe_glass_effect( "ff_ppe_glass_effect", "1",0,"HEV Suit Lens effect postprocessing. 0 = Off / 1 = Simple effect / 2 = Costly effect. NEEDS HDR ENABLED!" );
static ConVar ff_ppe_desaturate( "ff_ppe_desaturate", "0",0,"Desaturate the picture for a film noir effect" );
static ConVar ff_ppe_bokehdof( "ff_ppe_bokehdof", "0",0,"Activate Depth of Field with Autofocus and Bokeh Blur. 0 = Off / 1 = Variant 1 / 2 = Variant 2 / 3 = Stronger" );
static ConVar ff_ppe_gaussblur( "ff_ppe_gaussblur", "0",0,"1 = Enable 2D gaussian blur filter / 2 = Enable horizontal gaussian blur filter only" );
static ConVar ff_ppe_artyflare( "ff_ppe_artyflare", "0",0,"Add a artistic bloom/flare effect" );
static ConVar ff_ppe_dragonborn( "ff_ppe_dragonborn", "0",0,"Add the dragonborn color effect" );
static ConVar ff_ppe_filmgrain( "ff_ppe_filmgrain", "0",0,"Simulate film grain effect" );
static ConVar ff_ppe_filmic( "ff_ppe_filmic", "0",0,"Applies some common color adjustments to mimic a more cinema-like look" );
static ConVar ff_ppe_uncharted( "ff_ppe_uncharted", "0",0,"Simulate human eye color perception with the well known uncharted2 filmic curve. 0 = Off / 1 = on / 2 = on and stronger / 3 = on and strongest" );
static ConVar ff_ppe_sharpen( "ff_ppe_sharpen", "0",0,"Sharps the image but may increase aliasing. 0 = Off / 1 = on / 2 = on and stronger / 3 = on and strongest" );
static ConVar ff_ppe_fxaa( "ff_ppe_fxaa", "0",0,"Applies FXAA based antialiasing. Do not use with engine AA enabled." );
static ConVar ff_ppe_hpd( "ff_ppe_hpd", "0",0,"Apply Haarm Peter Duiker tonemapping. Used in Watch Dogs. 0 = Off / 1 = on / 2 = on and stronger / 3 = on and strongest" );
static ConVar ff_ppe_watchdog( "ff_ppe_watchdog", "0",0,"Enables one of the numerous watch dogs tonemapping algorithms. 0 = Off / 1 = on" );
//static ConVar ff_ppe_sincity( "ff_ppe_sincity", "0",0,"Enables Sincity effect. 0 = Off / 1 = on" );
//static ConVar ff_ppe_cartoon( "ff_ppe_cartoon", "0",0,"Enables TOON effect. 0 = Off / 1 = on / 2 = stronger / 3 = strongest" );
static ConVar ff_ppe_bloom( "ff_ppe_bloom", "0",0,"Enables Bloom enhancer. 0 = Off / 1 = on" );
static ConVar ff_ppe_curves("ff_ppe_curves", "0", 0, "Contrast adjustments using S-curves. 0 = Off / 1 = on");

// FAKEFACTORY SHADER EDITOR

// *****************************
// FAKKEFACTORY NIGHTVISION
// *****************************
static ConVar ff_ppe_nightvision( "ff_ppe_nightvision", "0",0,"1 = Enable nightvision (only with HEV suit)" );
// *****************************
// FAKKEFACTORY NIGHTVISION
// *****************************
...
//static ConVar ae_vignette( "ae_vignette", "0", FCVAR_ARCHIVE );
// static ConVar ae_dof( "ae_dof", "0", 0 );
//
...
void DoEnginePostProcessing( int x, int y, int w, int h, bool bFlashlightIsOn, bool bPostVGui )
{
	...
	if ( ff_ppe_artyflare.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_artyflare", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
	...
	/*if ( ff_ppe_cartoon.GetInt() == 1 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_cartoon", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_cartoon.GetInt() == 2 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_cartoon2", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_cartoon.GetInt() == 3 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_cartoon3", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	//if ( ff_ppe_sincity.GetInt() )
	//{
	//	static IMaterial *pMat = materials->FindMaterial( "postproc_sincity", TEXTURE_GROUP_OTHER );
	//	if ( pMat )
	//	{
	//		UpdateScreenEffectTexture();
	//		pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_dragonborn.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_dragonborn", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_bokehdof.GetInt() == 1 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_bokehdof", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_bokehdof.GetInt() == 2 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_bokehdof2", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_bokehdof.GetInt() == 3 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_bokehdof3", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_bloom.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_softbloom", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_desaturate.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_desaturate", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_gaussblur.GetInt() == 1 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_gaussblur", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_gaussblur.GetInt() == 2 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_gaussblur_xonly", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_filmgrain.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_filmgrain", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_filmic.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_filmic", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_uncharted.GetInt() == 1)
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_uncharted", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_uncharted.GetInt() == 2)
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_uncharted2", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_uncharted.GetInt() == 3)
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_uncharted3", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_hpd.GetInt() == 1)
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_hpd", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
		...
		if ( ff_ppe_hpd.GetInt() == 2)
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_hpd2", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_hpd.GetInt() == 3)
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_hpd3", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_watchdog.GetInt() == 1)
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_watchdog", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if (ff_ppe_curves.GetInt() == 1)
	{
		static IMaterial *pMat = materials->FindMaterial("postproc_curves", TEXTURE_GROUP_OTHER);
		if (pMat)
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle(pMat, 0, 0, w, h,
	...
	if ( ff_ppe_sharpen.GetInt() == 1 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_sharpen", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_sharpen.GetInt() == 2 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_sharpen2", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_sharpen.GetInt() == 3 )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_sharpen3", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	if ( ff_ppe_fxaa.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_fxaa", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	// Effekte, nur wenn der HEV Suit angelegt ist
	// Glass-Effekt möglichst weit hinten, aber noch vor Vignette
	C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
	if  ( pPlayer->IsSuitEquipped() ) 
	{
		if ( ff_ppe_glass_effect.GetInt() == 1)
		{
			static IMaterial *pMat = materials->FindMaterial( "postproc_glass_effect", TEXTURE_GROUP_OTHER );
			if ( pMat )
			{
				UpdateScreenEffectTexture();
				pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
		...
		if ( ff_ppe_nightvision.GetInt() == 1)
		{
			static IMaterial *pMat = materials->FindMaterial( "postproc_nightvision", TEXTURE_GROUP_OTHER );
			if ( pMat )
			{
				UpdateScreenEffectTexture();
				pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
		...
		if ( ff_ppe_glass_effect.GetInt() == 2)
		{
			static IMaterial *pMat = materials->FindMaterial( "postproc_glass_effect_2", TEXTURE_GROUP_OTHER );
			if ( pMat )
			{
				UpdateScreenEffectTexture();
				pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
	...
	// Vignette sollte ganz an das Ende der Renderpipeline, damit die feinen Übergänge erhalten bleiben
	if ( ff_ppe_vignette.GetInt() )
	{
		static IMaterial *pMat = materials->FindMaterial( "postproc_vignette", TEXTURE_GROUP_OTHER );
		if ( pMat )
		{
			UpdateScreenEffectTexture();
			pRenderContext->DrawScreenSpaceRectangle( pMat, 0, 0, w, h,
